...
**Læsning til `/admin/brug` (reviewfund rettet i PR #39):** `GET /admin/api/usage` henter ALDRIG `upload_events` med et enkelt `.select()` — PostgREST/Supabase har en standard max-rows-grænse (typisk 1000), som ville trunkere resultatet stille (ingen fejl, bare for lave tal) så snart tabellen passerer den grænse. I stedet henter `fetchAllUploadEvents()` (`src/lib/usage.ts`) ALLE rækker via keyset-paginering på `id` (`where id > sidste_id order by id limit N`, aldrig `OFFSET`), og stopper udelukkende ved en tom side — aldrig ved "færre rækker end forespurgt", som en skjult lavere server-grænse kunne give falsk indtryk af. Periodefiltrering (`summarizeUsage`) sammenligner `received_at` som epoch-ms (`Date#getTime()`), ikke som rå ISO-strenge, fordi Postgres kan returnere timestamptz i formater ("+00:00" vs. "Z", varierende mikrosekund-præcision) der ikke sorterer korrekt lexikografisk.
