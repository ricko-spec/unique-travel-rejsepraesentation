...
*States:* Aktiv sektion sat af en `IntersectionObserver` med `rootMargin: "-25% 0px -55% 0px"` (smalt trigger-bånd midt i viewporten — forhindrer flakken ved sektionsgrænser); klik beregner offset og bruger `window.scrollTo` (ikke `scrollIntoView`, jf. `docs/design/DO-NOT-CHANGE.md` §8), med `behavior: "auto"` i stedet for `"smooth"` når `prefers-reduced-motion: reduce`. Klik sætter desuden aktiv-state synkront med det samme (før scroll'en overhovedet starter), så feedbacket er øjeblikkeligt. Section-id'er (`intro`/`rejseplan`/`billeder`/`hoteller`/`pris`/`kontakt`) sidder direkte på Hero/Timeline/DestinationGallery/Hotels/PriceAndNote/ContactCTA's egne `<section>`-elementer — `kontakt` er uændret genbrug af det eksisterende anker som `ActionBar` og Hero's CTA'er allerede linker til.
*Reviewfund (PR #42):* den sidste sektion (typisk KONTAKT) kan sidde for tæt på sidens bund til nogensinde selv at krydse observer-båndet — der er ikke scroll-plads nok efter den. `isScrolledToBottom()` (`src/lib/progress-nav.ts`, ren funktion, unit-testet) tjekkes derfor både inde i IO-callbacket og fra en supplerende scroll/resize-lytter, og vinder altid når brugeren reelt er ved bunden af siden — uafhængigt af hvilken af de to der fyrer sidst efter en scroll. De øvrige sektioners bånd-baserede opførsel er urørt.
...
| Kolonne | Type | Betydning |
|---|---|---|
| `id` / `received_at` / `updated_at` / `completed_at` | uuid PK / timestamptz × 3 | `completed_at` sat ved `published` |
| `user_id` | uuid → `auth.users`, `on delete set null` | Sælgeren; `set null` bevarer eventet hvis brugeren slettes |
| `actor_name` | text | Snapshot af `profiles.full_name` (fallback email/id) på upload-tidspunktet |
| `status` | text (check) | `received` \| `validation_failed` \| `parse_failed` \| `parsed` \| `published` \| `save_failed` |
| `file_size_bytes` | bigint | — |
| `booking_no_hash` | text | sha-256(bookingNo) — **aldrig klartekst** (booking-nr = kundens adgangskode) |
| `trip_id` | uuid → `trips`, `on delete set null` | Sat ved `published` |
| `save_kind` | text (check) | `created` \| `updated` — spejler `wasUpdate` i `POST /admin/api/trips` |
| `failure_kind` | text (check) | Sanitiseret fejlkategori, aldrig rå fejltekst/kundedata |
