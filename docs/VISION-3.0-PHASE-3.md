...
**Eligibility** (`computeEligibleChannels`, på `tripSchema` + `normalizeTrip`-output): `email` kun
...
6. Fulde checks + Vercel.
7. ChatGPT final HEAD-review → Rickos merge-godkendelse → merge/deploy.
8. Production-smoketest når Ricko har mulighed (må ikke oprette kunstige events; udskydelse blokerer ikke merge).
