...
| **HubSpot** (fremtidig, Gate B/C — Issue #80) | `HUBSPOT_PRIVATE_APP_TOKEN` + `HUBSPOT_DEAL_KEY_SECRET` som **production-secrets i DETTE Vercel-projekt** (ADR A, `docs/VISION-3.0-PHASE-5-GATE-B0-ADR.md`) — **IKKE oprettet endnu**. Gate B1 (PR #81, merget) indeholder kun adapter-KONTRAKTEN (`src/lib/conversion/hubspotAdapter.ts`) + en fixture — **ingen agent foretager noget live HubSpot-kald i Gate B1** | Samme read-only scope som ovenfor, men server-side (Vercel Cron → sync-motor), ikke operatør-kørt. Oprettes/aktiveres først ved Gate C, eksplicit Ricko-godkendelse | Nej — ingen writes, samme princip som Gate A-rækken | Se `docs/VISION-3.0-PHASE-5-GATE-B1-RUNBOOK.md` for den fulde aktiveringsprocedure. Pipelinens komplette stage-liste (til `PIPELINE_STAGE_CONTRACT`, Gate C trin 1) hentes operatør-kørt af Ricko under en NY, eksplicit godkendelse — Gate A-godkendelsen dækker ikke dette. Kun stage-id'er/-navne må committes. **Gate C1 (Issue #84), godkendt af Ricko 2026-09-24:** operatør-kørt read-only pipeline-/property-metadata og ÉN read-only deal-search-dry-run for pipeline `754595640` fra Rickos eget shell (SecureString; agenten ser aldrig tokenet); kun aggregeret, small-cell-beskyttet output. Ikke omfattet: HubSpot-writes, Supabase-writes, Vercel-secrets, cron, `ACTIVE`, Gate D |
